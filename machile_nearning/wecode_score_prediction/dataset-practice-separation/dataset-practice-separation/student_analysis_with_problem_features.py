...
MIN_CLASS_SIZE = 10
SCORE_THRESHOLD = 8000  # Minimum score to consider as success
STUDENT_PERCENTAGE_THRESHOLD = 0.15  # Less than 5% of students must succeed (increased from 5%)
...
def export_class_statistics(df, class_assignments):
    ...
    # Define practice assignment ID and exclude it
    PRACTICE_ASSIGNMENT_ID = "1bec7c0b6a9bd8f556a8554c5012dcb778460bac"
    df = df[df['assignment_id'] != PRACTICE_ASSIGNMENT_ID].copy()
...
def calculate_metrics(df, class_assignments, qt_scores=None, th_scores=None, ck_scores=None):
    ...
    # Define practice assignment ID
    PRACTICE_ASSIGNMENT_ID = "1bec7c0b6a9bd8f556a8554c5012dcb778460bac"
    
    # Separate practice assignment data for later processing
    practice_df = df[df['assignment_id'] == PRACTICE_ASSIGNMENT_ID].copy()
    
    # Filter out practice assignment from normal statistics
    df = df[df['assignment_id'] != PRACTICE_ASSIGNMENT_ID].copy()
    
    # Get truly optional problems (max coefficient is 0) from normal assignments
    optional_problems = get_optional_problems(df)
    ...
    # Calculate practice assignment metrics
    if not practice_df.empty:
        practice_df = practice_df.merge(class_assignments, on='student_id', how='inner')
        practice_final = practice_df[practice_df['is_final'] == 1].copy()
        
        if not practice_final.empty:
            # Calculate practice wecode score
            practice_final['practice_problem_score'] = (
                practice_final['pre_score'] * practice_final['coefficient'] / 100
            )
            
            practice_problems_by_class = (
                practice_final.groupby('class_id')['problem_id']
                .nunique()
                .reset_index(name='practice_n_problems')
            )
            
            practice_wecode_scores = (
                practice_final.groupby(['student_id', 'class_id'])
                .agg(practice_total_score=('practice_problem_score', 'sum'))
                .reset_index()
            )
            
            practice_wecode_scores = practice_wecode_scores.merge(
                practice_problems_by_class, on='class_id', how='left'
            )
            
            practice_wecode_scores['practice_wecode_score'] = (
                practice_wecode_scores['practice_total_score'] / 
                practice_wecode_scores['practice_n_problems']
            ).fillna(0)
            
            # Calculate practice wecode score scaled
            practice_wecode_scores['practice_wecode_score_scaled'] = (
                practice_wecode_scores.groupby('class_id')['practice_wecode_score']
                .transform(lambda x: x / x.max() if x.max() > 0 else 0)
            )
            
            # Calculate practice success and error metrics
            practice_success_count = (
                practice_df[practice_df['pre_score'] >= SCORE_THRESHOLD]
                .groupby('student_id')
                .size()
                .reset_index(name='practice_success_count')
            )
            
            practice_error_count = (
                practice_df[practice_df['status'] == 'Compilation Error']
                .groupby('student_id')
                .size()
                .reset_index(name='practice_error_count')
            )
            
            practice_submit_count = (
                practice_df.groupby('student_id')
                .size()
                .reset_index(name='practice_submit_count')
            )
            
            # Merge practice metrics
            practice_metrics = practice_submit_count.merge(
                practice_success_count, on='student_id', how='left'
            )
            practice_metrics = practice_metrics.merge(
                practice_error_count, on='student_id', how='left'
            )
            practice_metrics = practice_metrics.fillna(0)
            
            practice_metrics['practice_success_rate'] = (
                practice_metrics['practice_success_count'] / 
                practice_metrics['practice_submit_count']
            ).fillna(0)
            
            practice_metrics['practice_error_rate'] = (
                practice_metrics['practice_error_count'] / 
                practice_metrics['practice_submit_count']
            ).fillna(0)
            
            practice_metrics['practice_avg_attempt'] = (
                practice_metrics['practice_submit_count'] / 
                practice_metrics['student_id'].map(
                    practice_df.groupby('student_id')['problem_id'].nunique()
                )
            ).fillna(0)
            
            # Merge practice scores and metrics
            practice_all_metrics = practice_wecode_scores.merge(
                practice_metrics, on='student_id', how='outer'
            ).fillna(0)
            
            # Merge practice metrics with main result
            result = result.merge(
                practice_all_metrics[['student_id', 'practice_wecode_score', 'practice_wecode_score_scaled',
                                     'practice_submit_count', 'practice_success_count', 'practice_error_count',
                                     'practice_success_rate', 'practice_error_rate', 'practice_avg_attempt']],
                on='student_id',
                how='left'
            )
    
    # Fill practice columns with 0 for students who didn't participate in practice assignment
    practice_columns = ['practice_wecode_score', 'practice_wecode_score_scaled',
                       'practice_submit_count', 'practice_success_count', 'practice_error_count',
                       'practice_success_rate', 'practice_error_rate', 'practice_avg_attempt']
