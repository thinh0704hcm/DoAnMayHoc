...
def predict_scores():
    ...
    th_path = os.path.join(VOLUME_PATH, "data", "th-public.csv")
    
    all_features = pd.read_csv(features_path)
    th_df = pd.read_csv(th_path).rename(columns={'hash': 'student_id', 'TH': 'exam_score'})
    ...
    # Load trained model
    model_path = os.path.join(VOLUME_PATH, "models", "th_student_score_predictor_optuna_xgb.pkl")
    ...
    output_path = os.path.join(VOLUME_PATH, "predictions", "th_unscored_predictions.csv")
