...
def predict_scores():
    ...
    qt_path = os.path.join(VOLUME_PATH, "data", "qt-public.csv")
    
    all_features = pd.read_csv(features_path)
    qt_df = pd.read_csv(qt_path).rename(columns={'hash': 'student_id', 'diemqt': 'exam_score'})
    qt_df['exam_score'] = pd.to_numeric(qt_df['exam_score'], errors='coerce')
    
    # Identify unscored students
    scored_students = set(qt_df['student_id'].dropna())
    ...
    # Load trained model
    model_path = os.path.join(VOLUME_PATH, "models", "qt_student_score_predictor_optuna_xgb.pkl")
    ...
    output_path = os.path.join(VOLUME_PATH, "predictions", "qt_unscored_predictions.csv")
