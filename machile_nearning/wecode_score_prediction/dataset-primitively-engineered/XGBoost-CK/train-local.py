...
TARGET_FILE = "ck-public.csv"
MODEL_PATH = "ck_student_score_predictor_optuna_xgb_local.pkl"

def train_xgboost():
    # Load data
    features = pd.read_csv(os.path.join(DATA_PATH, FEATURES_FILE))
    loaded_df = pd.read_csv(os.path.join(DATA_PATH, TARGET_FILE))
    loaded_df = loaded_df.rename(columns={'hash': 'student_id', 'CK': 'exam_score'})
    ...
    study.optimize(objective, n_trials=10, timeout=3600)  # 500 trials or 1 hour
