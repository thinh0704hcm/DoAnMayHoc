...
import joblib
import shap  # Add this import at the top
...
MODEL_PATH = "th_student_score_predictor_optuna_xgb_scaled.pkl"
...
def train_xgboost():
    ...
    features = [
        ...
        'hard_solved_scaled',     # Hard problems solved (scaled)
        'n_class_problems',
        'submit_count',
        'error_rate',
        'avg_attempt'
    ...
    train_data = student_results.dropna(subset=['th_score'])
    X = train_data[features]
    y = train_data['th_score']
    ...
    plt.tight_layout()
    plt.savefig("shap_summary_global_unscaled.png")
    plt.close()
    
    # # Individual prediction explanation (first sample)
    # plt.figure(figsize=(12, 6))
    # shap.decision_plot(explainer.expected_value, shap_values[0], 
    #                    features=X_valid.iloc[0], feature_names=features)
    # plt.savefig("shap_individual_example.png")
    # plt.close()
