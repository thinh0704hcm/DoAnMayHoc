...
# --- Prediction Function ---
@app.function(
    gpu="T4",  # Using A10G like training script
    volumes={VOLUME_PATH: volume},
    timeout=60*60*2  # 2 hours for larger model
)
def predict(data_path="/vol/evaluation_data", output_csv_path="/vol/efficientnet_b7_predictions.csv"):
    ...
    print(f"Best accuracy from training: {checkpoint.get('best_acc', 'N/A'):.4f}")
    ...
    for image_file in sorted(os.listdir(data_path)):
        ...
        try:
            # Load and preprocess image (supports HEIC/HEIF now)
...
def main():
    ...
    predict.remote("/vol/evaluation_data", "/vol/efficientnet_b7_predictions.csv")
