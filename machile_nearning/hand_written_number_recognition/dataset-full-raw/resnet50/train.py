...
# Định nghĩa thư mục gốc nơi dữ liệu đã được giải nén
data_root = "/content/sorted_data_extracted/sorted_data"

# Lớp Dataset tuỳ chỉnh
...
def load_and_split_data(data_root, train_transforms, test_transforms, train_split=0.8, batch_size=64, test_batch_size=1000):
    """
    Tải bộ dữ liệu, chia thành tập huấn luyện và kiểm tra, và tạo các DataLoader.

    Tham số:
        data_root (str): Thư mục chứa tất cả các thư mục số.
        train_transforms (callable): Phép biến đổi cho tập huấn luyện.
        test_transforms (callable): Phép biến đổi cho tập kiểm tra.
    ...
    Trả về:
        tuple: Một tuple gồm train_loader và test_loader.
    """
    try:
...
def build_model(num_classes, device):
    ...
    try:
        ...
        print("Đã tải mô hình ResNet50 pretrained.")
...
def train(model, train_loader, test_loader, optimizer, criterion, num_epochs, device, model_save_path="handwritten_digit_model_weights.pth", optimizer_save_path="handwritten_digit_optimizer_state.pth"):
    ...
    Tham số:
        ...
        model_save_path (str): Đường dẫn lưu trọng số mô hình.
        optimizer_save_path (str): Đường dẫn lưu trạng thái optimizer.
    """
    ...
    try:
        ...
        print(f"Đã lưu trạng thái optimizer tại {optimizer_save_path}")
    except Exception as e:
...
train_transforms = transforms.Compose([
    ...
    transforms.RandomAffine(
        degrees=15,  # Xoay ảnh
        translate=(0.1, 0.1),  # Dịch chuyển tối đa 10% theo cả hai chiều
        scale=(0.9, 1.1)  # Phóng to/thu nhỏ nhẹ cho đa dạng kích thước
    ),
    transforms.RandomPerspective(distortion_scale=0.2, p=0.5),  # Mô phỏng biến dạng góc nhìn
    transforms.ToTensor(),
    transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))  # Chuẩn hóa RGB
])

test_transforms = transforms.Compose([
    transforms.Resize((224, 224)),   # Đảm bảo kích thước đầu vào
    transforms.ToTensor(),
...
if train_loader is not None and test_loader is not None:
    ...
    if model is not None:
        ...
        # Kiểm tra và tải trọng số mô hình nếu có
        ...
        if os.path.exists(optimizer_save_path):
            try:
                ...
                print(f"Đã tải trạng thái optimizer từ {optimizer_save_path}")
            except Exception as e:
                print(f"Lỗi khi tải trạng thái optimizer: {e}")

        # Huấn luyện và đánh giá mô hình
        num_epochs = 12 # Số epoch có thể điều chỉnh
        ...
        print("Xây dựng mô hình thất bại. Bỏ qua huấn luyện.")
